let _token = "IGQVJXWWJxeVljSnRhYkpOVHJzMktSTXhsS3JWOXJUWTk3Qk9kcWQtVERtRUZALVmczcnZASMnF1dW5rUjBiVU5EdWJWNWs4WXJEUEo4NExCZAV83WklyRXlxUXRfRFJCUVJEcV9zMGVJTGo2VjVaVFd4UAZDZD";
    ...
    const result = data.map(info => {
        ...
        return `<a href="${info.permalink}" target='_blank' class="reviews__item">
            ...
            <div class="reviews__description">
                <p class="reviews__text">${lastSpace > 0 ? info.caption.slice(0, lastSpace) : textSlice}...</p>
...
$.ajax({
    ...
    success: response => {
        $.ajax({
            url: `https://graph.instagram.com/me/media?fields=media_url,timestamp,permalink,media_type,thumbnail_url,caption&edges=media&access_token=${response.access_token}`,
            dataType: 'jsonp',
            type: 'GET',
            success: data => {
